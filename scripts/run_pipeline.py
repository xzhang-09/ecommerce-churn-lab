...
from src.data.preprocess import impute_numeric            # Leakage-free median imputation
from src.features.build_features import (
    apply_feature_schema,
    build_feature_schema,
)  # Feature engineering (CRITICAL for model performance)
from src.models.tune import tune_model, build_tuned_estimator  # Optional Optuna hyperparameter search
...
def cross_val_oof(estimator, X, y, cv: int = 5, es_rounds: int = 50,
    ...
    for fit_idx, pred_idx in skf.split(X, y):
        ...
        X_sub, X_es, y_sub, y_es = train_test_split(
            X_fold, y_fold, test_size=0.2, stratify=y_fold, random_state=random_state
        )
...
def compare_models(
    ...
    The single-split test precision/recall/f1/cost are still reported per model
    for context (each model's threshold is chosen on its out-of-fold predictions),
    but they do NOT drive the ranking.
    """
    ...
    for spec in get_model_specs(scale_pos_weight=scale_pos_weight, random_state=random_state):
        ...
        # Threshold from out-of-fold predictions (stable across the whole pool),
        # then a single-split fit for the test operating point shown for context.
        threshold, _ = select_threshold(
            y_trainval_pos, oof, metric=threshold_metric, fn_fp_ratio=fn_fp_ratio
        )
        fitted = fit_with_early_stopping(
            clone(spec.estimator), X_train, y_train, X_val, y_val
        )
        operating = metrics_at_threshold(y_test, predict_positive_probability(fitted, X_test), threshold)
        ...
        rows.append({
            ...
            "f1": operating["f1"],
            "test_cost": expected_cost(operating, fn_fp_ratio),
            "cv_time": cv_time,
        })
        ...
        with mlflow.start_run(run_name=f"compare_{spec.name}", nested=True):
            ...
            mlflow.log_metric("test_cost", expected_cost(operating, fn_fp_ratio))
    ...
    print(f"\n📊 Model comparison — ranked by CV PR-AUC (mean ± std over folds); "
          f"precision/recall/f1 and test_cost (FN={fn_fp_ratio}xFP) are single-split "
...
def run_data_validation(df, skip_validation: bool = False):
    ...
    try:
        from src.utils.validate_data import validate_ecommerce_data
...
def main(args):
    ...
    with mlflow.start_run():
        ...
        oof_proba, cv_stats = cross_val_oof(model, X_trainval, y_trainval, cv=5)
        ...
        t1 = time.time()
        proba = model.predict_proba(X_test)[:, 1]  # Probability of churn (class 1)
        ...
        # Reported on the test set and (more stably) on the OOF predictions. With
        # class reweighting in play these probabilities are expected to be poorly
        # calibrated — this quantifies it so anyone using the probability itself
        # (risk ranking, expected-value targeting) knows whether to trust it or
        # calibrate first. Calibration does not affect the threshold decisions
        # above (it is a monotonic rescaling and preserves ranking/PR-AUC).
        ...
        if ece_oof > 0.05:
            print("   ⚠️  ECE > 0.05 — probabilities are not well calibrated; "
                  "calibrate (e.g. isotonic/Platt) before using them as true probabilities.")

        # === STAGE 7: Model Serialization and Logging ===
        print("💾 Saving model to MLflow...")
        # Log model in MLflow's standard format for reproducible analysis.
...
if __name__ == "__main__":
    ...
    p.add_argument("--compare_models", action="store_true",
                   help="train and log baseline model comparisons before the final XGBoost run")
...
# Or with an Optuna hyperparameter search instead of the fixed defaults:

python scripts/run_pipeline.py \
    --input "data/raw/E Commerce Dataset.xlsx" \
    --target Churn \
    --tune --tune_metric average_precision --tune_trials 20
